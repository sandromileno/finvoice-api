...
RSpec.describe "Api::V1::PaymentsController#create", :type => :request do
  subject do
    post "/api/v1/customers/#{invoice.customer.key}/invoices/#{invoice.key}/payments", params: body, headers: headers
    response
  end

  let(:user) { build(:create_user_request) }

  context 'when create a payment' do
    before do
    ...
    let(:body) { build(:create_payment_request) }
  ...
  context 'when create a payment with a invalid invoice id' do
    before do
      post '/sign_up', params: user
      post '/sign_in', params: user
    end
  ...
  context 'when unexpected error on create new payment' do
    before do
      post '/sign_up', params: user
      post '/sign_in', params: user
